...
class WMSA(nn.Module):
    ...
    def forward(self, x):
        ...
        sim = sim + self.relative_embedding()
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, num_classes, config=[2,2,6,2], dim=96, drop_path_rate=0.2):
        ...
        begin = 0
        self.stage1 = [nn.Linear(48, dim),] + \
                      [Block(dim, dim, self.head_dim, self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW') 
                      for i in range(config[0])]
        begin += config[0]
        self.stage2 = [Rearrange('b (h neih) (w neiw) c -> b h w (neih neiw c)', neih=2, neiw=2), nn.Linear(4*dim, 2*dim),] + \
                      [Block(2*dim, 2*dim, self.head_dim, self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW')
                      for i in range(config[1])]
        begin += config[1]
        self.stage3 = [Rearrange('b (h neih) (w neiw) c -> b h w (neih neiw c)', neih=2, neiw=2), nn.Linear(8*dim, 4*dim),] + \
                      [Block(4*dim, 4*dim, self.head_dim, self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW')
                      for i in range(config[2])]
        begin += config[2]
        self.stage4 = [Rearrange('b (h neih) (w neiw) c -> b h w (neih neiw c)', neih=2, neiw=2), nn.Linear(16*dim, 8*dim),] + \
                      [Block(8*dim, 8*dim, self.head_dim, self.window_size, dpr[i+begin], 'W' if not i%2 else 'SW')
